...
class CustomPad:
    ...
    def has_total_amount(self):
        return len(self._entry.values) > 2 and self._entry.values[2].dtype == amount.Amount
...
def create_pads(
    ...
    *,
    ...
    dates = [dt for i, dt in enumerate(date_utils.iter_dates(start, end)) if i % gap == gap - 1]
    ...
    for idx, current_date in enumerate(dates, start=1):
        ...
        r.append(
            data.Transaction(
                dict(**meta), current_date, flags.FLAG_PADDING, None, narration, data.EMPTY_SET, data.EMPTY_SET, [
                    data.create_simple_posting(None, account, amount_.number, amount_.currency),
                    data.create_simple_posting(None, source_account, -amount_.number, amount_.currency),
                ]
            )
        )
    return r
...
def spread_pad(entries, options_map):
    ...
    pad = [CustomPad(e) for e in misc_utils.filter_type(entries, data.Custom) if e.type == 'pad']
    ...
    for account in pad_dict.keys() & by_account.keys():
        ...
        for entry_or_txn_posting in by_account[account]:
            ...
            elif isinstance(entry_or_txn_posting, data.Balance) and eligable_pad is None:
                first_directive = entry_or_txn_posting
            elif isinstance(entry_or_txn_posting, data.Balance) and eligable_pad is not None and account == eligable_pad.account:
                try:
                    for e in create_pads(
                            first_directive.date,
                            entry_or_txn_posting.date,
                            account_balance.get_currency_units(entry_or_txn_posting.amount.currency),
                    ...
                    errors.append(SpreadPadError(eligable_pad.meta, e, entry_or_txn_posting))
                first_directive = entry_or_txn_posting
                eligable_pad = None
